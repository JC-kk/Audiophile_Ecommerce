...
export default function DigitalAudioPlayersPage() {
  const [sortBy, setSortBy] = useState('name');
  const [sortOrder, setSortOrder] = useState('asc');

  const players = products.filter(product => 
    product.category === 'dacs-amps' && 
    product.subcategory === 'players'
  );

  const sortedProducts = [...players].sort((a, b) => {
    if (sortBy === 'price') {
  ...
  return (
    <div className="container mx-auto px-4 pt-24 pb-16">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">数字音频播放器</h1>
