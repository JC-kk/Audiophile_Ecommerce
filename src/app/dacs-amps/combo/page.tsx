...
export default function ComboDacsAmpsPage() {
  const [sortBy, setSortBy] = useState('name');
  const [sortOrder, setSortOrder] = useState('asc');

  const comboDacsAmps = products.filter(product => 
    product.category === 'dacs-amps' && 
    product.subcategory === 'combo'
  );

  const sortedProducts = [...comboDacsAmps].sort((a, b) => {
    if (sortBy === 'price') {
  ...
  return (
    <div className="container mx-auto px-4 pt-24 pb-16">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">DAC/放大器一体机</h1>
